<?php
    ...
    include "accessFolder.php";

    include "database/database.php";
    $sql = "SELECT COUNT(`ID`) AS `OGRENILDI` FROM `words` WHERE `LEARNED` = 0 AND `USERS_ID` = {$_SESSION['id']}";
    $query = mysqli_query($dbconnect, $sql);
    $learnedDataFetch = mysqli_fetch_assoc($query);
    $sql = "SELECT COUNT(`ID`) AS `OGRENILEN` FROM `words` WHERE `LEARNED` != 0 AND `USERS_ID` = {$_SESSION['id']}";
    $query = mysqli_query($dbconnect, $sql);
        ...
        <div class="right">
            ...
            <div class="myWords">
                <table border="0" cellspacing="0" cellpadding="0">
                    <?php
                        $Lsql = "SELECT * FROM `words` WHERE `LEARNED` = 1 and `USERS_ID` = {$_SESSION['id']}";
